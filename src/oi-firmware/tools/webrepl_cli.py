...
def do_command(ws, command, timeout_s=2.0):
    """Send a single command to the remote friendly REPL and print brief output.

    This is intentionally small: it is used by scripts/push.sh to issue
    `import machine; machine.soft_reset()` after file upload. Ctrl-C first so
    a running main.py drops back to the REPL, then send the command followed by
    carriage return. A reset may close the websocket before we read anything;
    that is considered success by the caller.
    """
    import select, time

    for c in b"\x03\x03":
        ws.write(bytes([c]), WEBREPL_FRAME_TXT)
        time.sleep(0.05)
    ...
    while time.time() < deadline:
        ...
        readable, _, _ = select.select([ws.s], [], [], remaining)
        if not readable:
            break
        try:
            c = ws.read(1, text_ok=True)
...
def help(rc=0):
    ...
    print("  [-p password] --command <cmd> <host>            - Run one REPL command")
...
def main():
    ...
    while i < len(sys.argv):
        if sys.argv[i] == '-p':
            ...
            continue
        if sys.argv[i] == '--command':
            sys.argv.pop(i)
            command = sys.argv.pop(i)
            continue
        i += 1
    ...
    if True:
        print("op:%s, host:%s, port:%d, passwd:***." % (op, host, port))
        if op in ("get", "put"):
