...
async def test_main_loads_config_builds_backend_and_runs(monkeypatch):
    ...
    catalog = MagicMock()
    catalog.default_backend_id = "dummy"
    catalog.get.return_value = backend
    fake_runtime = MagicMock(spec=GatewayRuntime)
    with patch("gateway_app.load_gateway_toml_config") as load_cfg, patch("gateway_app.create_backend_catalog_from_env", return_value=catalog) as create_catalog, patch("gateway_app._load_agent_catalog", return_value=({"id": "main", "name": "Main"}, [{"id": "main", "name": "Main"}])) as load_agents, patch("gateway_app.GatewayRuntime", return_value=fake_runtime) as runtime_cls, patch("gateway_app.run_forever", new=AsyncMock()) as run_loop:
